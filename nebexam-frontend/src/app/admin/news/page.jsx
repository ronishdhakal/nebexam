  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {!loading && (
          <span className="text-xs text-slate-400">{filtered.length} of {news.length}</span>
        )}
      </div>
      ...
      {!loading && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {filtered.length === 0 && <div className="text-center py-12 text-slate-400 text-sm">No news articles found.</div>}
        </div>
      )}
    </div>
  );
