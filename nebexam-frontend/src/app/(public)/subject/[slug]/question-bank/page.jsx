...
function EntryCard({ entry, subjectSlug }) {
  return (
    <Link
      href={`/subject/${subjectSlug}/question-bank/${entry.slug}`}
...
export default async function QuestionBankListPage({ params }) {
  ...
  try {
    const res = await entriesService.getAll({ subject: slug });
        ...
        <section>
          ...
          <div className="rounded-xl border border-gray-100 dark:border-slate-700 overflow-hidden divide-y divide-gray-100 dark:divide-slate-700">
            {oldQuestions.map((entry) => (
              <EntryCard key={entry.id} entry={entry} subjectSlug={slug} />
            ))}
          </div>
        </section>
      )}
