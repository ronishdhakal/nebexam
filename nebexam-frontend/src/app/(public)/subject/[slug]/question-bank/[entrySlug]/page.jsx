...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    return { title: `${parts.join(' ')} — NEB Exam` };
  } catch {
...
export default async function EntryPage({ params }) {
  ...
  try {
    const [entryRes, subjectRes, allRes] = await Promise.all([
      entriesService.getOne(entrySlug),
      subjectsService.getOne(slug),
      entriesService.getAll({ subject: slug }),
    ]);
    entry = entryRes.data;
  ...
  // Sidebar: group all entries by type, sorted by year desc
  ...
  // Prev / next within the same type group, ordered by year
        ...
        <aside className="hidden lg:block w-60 xl:w-64 shrink-0 sticky top-20 self-start">
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
                  ...
                  <ul className="divide-y divide-gray-50 dark:divide-slate-700/50">
                    {modelQuestions.map((e) => {
                      ...
                      return (
                        <li key={e.id}>
                          <Link
                            href={`/subject/${slug}/question-bank/${e.slug}`}
                    ...
                    {oldQuestions.map((e) => {
                      ...
                      return (
                        <li key={e.id}>
                          ...
                          >
                            {isActive && (
                              <svg className="w-2.5 h-2.5 shrink-0 text-amber-500" fill="currentColor" viewBox="0 0 8 8">
                                <circle cx="4" cy="4" r="4"/>
                              </svg>
                            )}
                            <span className={`truncate ${isActive ? '' : 'pl-[18px]'}`}>{e.title}</span>
            ...
            <div className="px-4 py-2.5 border-t border-gray-100 dark:border-slate-700">
              <Link
                href={`/subject/${slug}/question-bank`}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-4">
            <div className="flex items-center gap-1.5 text-xs text-slate-400 flex-wrap">
              ...
              {subject && (
                <>
                  <Link href={`/class-${subject.class_level}`} className="hover:text-[#1CA3FD] transition-colors">Class {subject.class_level}</Link>
                  <span>/</span>
                  <Link href={`/subject/${slug}`} className="hover:text-[#1CA3FD] transition-colors">{subject.name}</Link>
                  <span>/</span>
                  <Link href={`/subject/${slug}/question-bank`} className="hover:text-[#1CA3FD] transition-colors">Question Bank</Link>
                  <span>/</span>
                </>
              )}
            ...
            <div className="px-4 sm:px-10 md:px-14 pt-6 sm:pt-8 pb-6 border-b-2 border-black dark:border-slate-400">
              ...
              <div className="mt-5 text-sm italic text-slate-700 dark:text-slate-300 leading-relaxed">
                ...
                {entry.has_disclaimer_2 && entry.disclaimer_2 && (
                  <div className="mt-2">
                    <RichTextRenderer content={entry.disclaimer_2} />
                  </div>
                )}
              </div>

              {/* Time / Marks / Pass Marks */}
              <div className="mt-5 pt-4 border-t border-gray-300 dark:border-slate-600">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <span className="font-bold text-sm">
                    {entry.time ? `समय${isNp ? '' : ' (Time)'} : ${entry.time}` : ''}
                  </span>
                  <span className="font-bold text-sm">
                    {entry.full_marks ? `पूर्णाङ्क${isNp ? '' : ' (Full Marks)'} : ${entry.full_marks}` : ''}
                  </span>
                </div>
                {entry.pass_marks && (
                  <p className="text-xs text-slate-500 mt-1 text-right">
                    {isNp ? 'उत्तीर्णाङ्क' : 'Pass Marks'} : {entry.pass_marks}
                  </p>
                )}
              </div>
          ...
          {(prev || next) && (
            <div className="mt-6 grid grid-cols-2 gap-3">
              {prev ? (
                <Link
                  href={`/subject/${slug}/question-bank/${prev.slug}`}
                ...
                >
                  <div className="w-8 h-8 rounded-full border border-gray-200 dark:border-slate-600 flex items-center justify-center shrink-0 group-hover:bg-[#1CA3FD] group-hover:border-[#1CA3FD] transition-all">
                    <svg className="text-slate-400 group-hover:text-white transition-colors" width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
              ...
              {next ? (
                <Link
                  href={`/subject/${slug}/question-bank/${next.slug}`}
...
function PaperGroup({ group, isNepali, startIndex = 0 }) {
  ...
  return (
    <div>
      {/* Group title — centered like real paper */}
      <div className="text-center mb-4">
        <h2 className="text-base font-extrabold underline underline-offset-2 tracking-wide">
          {group.group_title}
        </h2>
      </div>

      {/* Disclaimer + marks on same line */}
      {(group.group_disclaimer || group.marks != null || displayLabel) && (
        <div className="flex items-start justify-between gap-4 mb-4">
          <p className="text-sm text-slate-700 dark:text-slate-300 italic flex-1">
            {group.group_disclaimer || ''}
          </p>
          <div className="shrink-0 flex items-center gap-2">
            {group.marks != null && (
              <span className="font-bold text-sm">{group.marks} Marks</span>
            )}
            {displayLabel && (
