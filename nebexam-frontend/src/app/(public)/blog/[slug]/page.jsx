...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const item = res.data;
    const title = `${item.title} — NEB Exam Blog`;
...
export default async function BlogDetailPage({ params }) {
  ...
  try {
    const res = await blogService.getAll({ page_size: 6 });
    const all = res.data.results || res.data;
    recentPosts = all.filter((p) => p.slug !== slug).slice(0, 5);
  } catch {}
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: item.title, item: absoluteUrl(`/blog/${slug}`) },
    ],
  };
